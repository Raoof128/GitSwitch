    ...
    return (
      ...
      >
        <div className="flex items-start gap-3">
          <span
            className="mt-0.5 inline-flex h-4 w-4 animate-spin rounded-full border-2 border-[#00ffaa] border-t-transparent"
          />
          <div>
            <div className="text-sm font-semibold text-[#e0e0e0]">{activeTask.label}</div>
            {activeTask.detail && (
              <div className="mt-1 text-xs text-[#666666]">{activeTask.detail}</div>
            )}
          </div>
        </div>
        <div
    ...
    >
      <div className="min-w-0">
        <div
          ...
          style={{ color: textColor, textShadow: glow }}
        >
          {feedback.title}
        </div>
        {feedback.detail && <div className="mt-1 text-xs text-[#e0e0e0] opacity-90">{feedback.detail}</div>}
        {feedback.suggestion && (
          <div className="mt-2 text-[11px] text-[#e0e0e0]/90">
